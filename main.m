...
% PID gains
Kp = diag([14.908 ,  9.856  , 18.875]);
Kd = diag([17.480  , 11.554 , 20.4670]);
Kp_ang = diag([150, 150, 100]);
Kd_ang = diag([50, 50, 30]);
% TD3
% Kp = diag([3.7377  ,  9.9412  , 12.4008]);
% Kd = diag([6.9054  , 10.4579, 5.8498]);
% DDPG
% Kp = diag([17.2474 ,  12.6418  , 15.6788]);
% Kd = diag([12.1427  , 17.0238 , 13.3087]);
% PPO
% Kp = diag([14.908 ,  9.856  , 18.875]);
% Kd = diag([17.480  , 11.554 , 20.4670]);
